...
namespace StarterAssets
{
	...
	public class FirstPersonController : MonoBehaviour
	{
        ...
        {
			jumpButton.action.started += CheckJump;
        }
		...
		private void CheckJump(InputAction.CallbackContext callback)
		{
			if (Grounded)
			{
		...
		private void Move()
		{
			...
			{
                if (lastStepTime < Time.time - stepDelay)
                {
                    lastStepTime = Time.time;

                    //audioSource.PlayOneShot(stepsSound[Random.Range(0, stepsSound.Length)]);
                }
            }
		...
		{
			bool tempGround = Grounded;

			yield return new WaitForSeconds(0.01f);

			if(!tempGround && Grounded)
			{
